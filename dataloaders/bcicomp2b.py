...
class BCICompIV2b():
    ...
    def __init__(self, root, sub, memory, task_num, train=True):
        ...
        # data_key = 'trainX'
        # target_key = 'trainY'
        # raw_data = np.transpose(np.array(data_dict[data_key]), (2, 1, 0))
        # raw_targets = list(np.squeeze(np.array(data_dict[target_key])))
        #
        ...
        # self.nchans, self.ntimes = self.data.shape[1], self.data.shape[2]
        #
        # self.tt = [task_num] * len(self.data)                                                             # task module labels
        # self.td = [task_num + 1] * len(self.data)

        self.train = train
        self.root = root
        data_dict = scipy.io.loadmat(self.root + 'new_train/B' + str(sub) + '.mat')  # load training data
        ...
        # if train and memory is not None:
        #     for task_id in range(task_num):
        #         mem_data = np.array(memory[task_id]['x'])
        #         mem_targets = memory[task_id]['y']
        #         mem_tt = memory[task_id]['tt']
        #         mem_td = memory[task_id]['td']
        #         print(f"self.data shape: {self.data.shape}")
        #         print(f"mem_data shape: {mem_data.shape}")
        #
        #         # 确保记忆数据的格式与当前数据集一致
        #
        #         # Check if mem_data is not empty before concatenating
        #         if mem_data.size > 0:
        #             self.data = np.concatenate((self.data, mem_data), axis=0)
        #             self.targets.extend(mem_targets)
        #             self.tt.extend(mem_tt)
        #             self.td.extend(mem_td)
...
class DatasetGen(object):
    ...
    def get(self, task_id):
        ...
        self.train_set[task_id] = BCICompIV2b(root=self.root, sub=self.subject_queue[task_id], memory=memory, task_num=task_id)
        ...
        self.dataloaders[task_id]['name'] = f'BCI-COMP2B-task{task_id}-sub{self.subject_queue[task_id]}'
    ...
    def update_memory(self, task_id):
        ...
        for i in range(len(self.task_ids[task_id])):
            dataset = BCICompIV2b(root=self.root, sub=self.subject_queue[task_id], memory=memory, task_num=task_id, train=True)
