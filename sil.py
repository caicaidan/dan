...
class SIL(object):
    ...
    def __init__(self, model, args, network):
        ...
        if not args.experiment == 'multidatasets':
            self.e_lr=[args.e_lr] * args.num_subjects
            self.d_lr=[args.d_lr] * args.num_subjects
        else:
        ...
        self.s_steps=args.s_step # s_steps is the number of steps to train the shared module == 100
        self.d_steps=args.d_step # d_steps is the number of steps to train the discriminator == 100

        self.diff=args.diff # yes
        ...
        self.diff_loss= DiffLoss().to(self.device) # Orthogonal loss for shared and task-specific features
    ...
    def get_D_optimizer(self, task_id, d_lr=None):
        ...
        optimizer_D = torch.optim.Adam(self.discriminator.parameters(), lr=d_lr, weight_decay=self.args.d_wd)
        return optimizer_D

    def train(self, task_id, dataset):
        self.discriminator=self.get_discriminator(task_id)

        best_loss=np.inf  #表示正无穷，没有确切的值，类型为浮点数
        ...
        dis_lr_update=True
        d_lr=self.d_lr[task_id] # d_lr is the learning rate of discriminator
        patience_d=self.lr_patience
        self.optimizer_D=self.get_D_optimizer(task_id, d_lr)

        e_lr=self.e_lr[task_id] # e_lr is the learning rate of encoder
    ...
    def eval_(self, data_loader, task_id):
        ...
        self.discriminator.eval() # Sets the module in evaluation mode.
    ...
    def get_tsne_embeddings_first_ten_tasks(self, dataset, model):
        ...
        for t in range(10):
            for itr, (data, _, tt, td) in enumerate(dataset[t]['tsne']):
                ...
                shared_out, private_out = model.get_encoded_ftrs(x, x, t) # (x_s,x_p,task_id)
...
# # Test final model on last 3 tasks
    def get_tsne_embeddings_last_three_tasks(self, dataset, model): # not found usage in all pages
        from tensorboardX import SummaryWriter

        # Test final model on last 3 tasks:
        model.eval()
        tag = '_diff_{}'.format(self.args.diff)
        nsubs = self.args.num_subjects  # 9
        for t in range(nsubs-3, nsubs):  # 6, 7, 8
            ...
            writer.add_embedding(mat=torch.stack(all_shared,dim=1).data, label_img=torch.stack(all_images,dim=1).data,
                             metadata=list(range(1,6)), tag='Shared_{}_{}'.format(t, tag)) # ,metadata_header=list(range(1,6)))
            writer.add_embedding(mat=torch.stack(all_private,dim=1).data, label_img=torch.stack(all_images,dim=1).data,
                             metadata=list(range(1,6)), tag='Private_{}_{}'.format(t, tag)) # ,metadata_header=list(range(1,6)))
            writer.close()
